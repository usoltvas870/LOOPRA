...
class CookieRefreshFileTests(unittest.TestCase):
    def setUp(self):
        ...
        self.cookie_path.write_text(json.dumps({'cookies': [{'name': 'sessionid', 'domain': '.tiktok.com'}], 'origins': []}), encoding='utf-8')
    ...
    def test_failed_first_or_second_check_preserves_cookie(self):
        original = refresh.file_sha256(self.cookie_path)
        refresh.write_storage_state(self.pending_path, {'cookies': [{'name': 'sessionid', 'domain': '.tiktok.com'}], 'origins': []})
    ...
    def test_valid_pending_is_atomically_promoted_only_after_two_checks(self):
        refresh.write_storage_state(self.pending_path, {'cookies': [{'name': 'sessionid', 'domain': '.tiktok.com'}, {'name': 'msToken', 'domain': '.www.tiktok.com'}], 'origins': []})
    ...
    def test_guest_tiktok_cookies_are_not_promoted_as_authenticated(self):
        valid, reason = refresh.validate_storage_state({'cookies': [{'name': 'msToken', 'domain': '.tiktok.com'}], 'origins': []})
        self.assertFalse(valid)
        self.assertEqual(reason, 'authenticated TikTok session cookies are missing')
